...
$(function(){socket=io.connect("http://MYSITE.COM:9090");2==nodechat[6]&&(nodechat[6]=1550<=$(window).width()?1:0);$("#nodechat_btn").click(function(){1==nodechat[0]?(nodechat_close(),nodechat[0]=0):(nodechat_open(),nodechat[0]=1);nodechat_setings_save();return!1});socket.on("chat_join",function(){"1"==nodechat[0]&&(nodechat_open(),$("#nodechat_btn").addClass("onChat"))});socket.on("chat_init",function(a){$("#nodechat").remove();1==nodechat[6]?$("body").prepend(a):$("#mainContent").prepend(a);nodechat_init()});
socket.on("chat_msg2client",function(a){a.info?($("#nodechat_msg_list").prepend(a.info),setTimeout(function(){$(".nodechat_info").fadeTo(750,0,function(){$("div").remove(".nodechat_info")})},5E3)):(a.del&&$("#nodechat_mess_"+a.del).remove(),a.message&&($("#nodechat_msg_list").prepend(a.message),5==dle_group&&$("a").remove(".nodechat_pm"),2<dle_group&&$("a").remove(".nodechat_mess_del")))})});function nodechat_open(){socket.emit("join2chat");$("#nodechat_btn").addClass("onChat")}
