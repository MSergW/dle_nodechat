...
$(function(){socket=io.connect("http://megafile.mksat.net:8080");2==nodechat[6]&&(nodechat[6]=1550<=$(window).width()?1:0);$("#nodechat_btn").click(function(){1==nodechat[0]?(nodechat_close(),nodechat[0]=0):(nodechat_open(),nodechat[0]=1);nodechat_setings_save();return!1});socket.on("chat_join",function(){"1"==nodechat[0]&&(nodechat_open(),$("#nodechat_btn").addClass("onChat"))});socket.on("chat_init",function(a){1==nodechat[6]?$("body").prepend(a):$("#mainContent").prepend(a);nodechat_init()});socket.on("chat_msg2client",
function(a){a.info?($("#nodechat_msg_list").prepend(a.info),setTimeout(function(){$(".nodechat_info").fadeTo(750,0,function(){$("div").remove(".nodechat_info")})},5E3)):(a.del&&$("#nodechat_mess_"+a.del).remove(),a.message&&($("#nodechat_msg_list").prepend(a.message),5==dle_group&&$("a").remove(".nodechat_pm"),2<dle_group&&$("a").remove(".nodechat_mess_del")))})});function nodechat_open(){socket.emit("join2chat");$("#nodechat_btn").addClass("onChat")}
function nodechat_close(){socket.emit("leave_chat");$("#nodechat_btn").removeClass("onChat");$("#nodechat").hide("slide",100,function(){$(this).remove()})}function nodechat_location(){}function nodechat_setings_save(){$.cookie("nodechat",nodechat.join("_"),{expires:365,path:"/"})}
...
c){nodechat[1]=c.size.height-2;nodechat[2]=c.size.width-2;nodechat[5]==1&&$("#nodechat").css({position:"fixed"});nodechat_messheight();nodechat_setings_save()}}).draggable({handle:"#nodechat_head",containment:"document",scroll:!1,stop:function(a,c){nodechat[3]=c.offset.top;nodechat[4]=c.offset.left;nodechat_setings_save()}}).offset({top:nodechat[3],left:nodechat[4]}).show("slide",100)):($("#nodechat").css({height:"300px"}),$("#nodechat_msg_list").css({height:"200px"}),$("#nodechat_icons_unlocked").hide(),
$("#nodechat_icons_locked").hide(),$("#nodechat").show("slide",100));$("#nodechat_icons li").hover(function(){$(this).addClass("ui-state-hover")},function(){$(this).removeClass("ui-state-hover")});$("#nodechat_icons_locked").click(function(){nodechat[5]=0;$("#nodechat").css({position:"absolute"});$("#nodechat_icons_locked").hide();$("#nodechat_icons_unlocked").show();nodechat_setings_save();return false});$("#nodechat_icons_unlocked").click(function(){$("#nodechat").css({position:"fixed"});$("#nodechat_icons_unlocked").hide();
$("#nodechat_icons_locked").show();nodechat[5]=1;nodechat_setings_save();return false});$("#nodechat_icons_close").click(function(){nodechat_close();nodechat[0]=0;nodechat_setings_save();return false});$("#nodechat_icons_newwin").click(function(){nodechat[6]=nodechat[6]=="1"?0:1;nodechat_setings_save();$("#nodechat").remove();nodechat_open();nodechat_init();return false})}function nodechat_messheight(){$("#nodechat_msg_list").css({height:nodechat[1]-106+"px"})}
